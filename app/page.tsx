...
const STATUS_ORDER = KANBAN_COLS.map(c => c.key)
  ...
  return (
    <main style={{ background: 'var(--bg)', color: 'var(--fg)', minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Geist Mono', monospace", fontWeight: 300 }}>
      ...
      <section style={{ padding: '4rem 2.5rem 2rem', maxWidth: '1200px', width: '100%', margin: '0 auto' }}>
        ...
        <h1 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 'clamp(2.4rem, 6vw, 4.5rem)', lineHeight: 1.05, letterSpacing: '-0.02em', marginTop: '0.75rem' }}>
          reels in <em style={{ fontStyle: 'italic', color: 'var(--muted)' }}>motion.</em>
        </h1>
      </section>

      {/* Aggregate stats */}
      <section style={{ maxWidth: '1200px', width: '100%', margin: '0 auto', padding: '0 2.5rem 2.5rem', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
        {[
      ...
      {/* View toggle */}
      <section style={{ maxWidth: '1200px', width: '100%', margin: '0 auto', padding: '0 2.5rem 1.5rem', display: 'flex', gap: '1rem' }}>
        {(['kanban', 'list'] as const).map(v => (
          <button key={v} onClick={() => setView(v)} style={{
            ...
            borderBottom: view === v ? '1px solid var(--fg)' : '1px solid transparent',
      ...
      {/* KANBAN VIEW */}
      {view === 'kanban' && (
        <section style={{ maxWidth: '1200px', width: '100%', margin: '0 auto', padding: '0 2.5rem 4rem', overflowX: 'auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${KANBAN_COLS.length}, minmax(180px, 1fr))`, gap: '1rem', minWidth: '900px' }}>
            {KANBAN_COLS.map(col => {
              const colReels = reelsData.filter(r => r.status === col.key)
              return (
                <div key={col.key}>
                  {/* Column header */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: `2px solid ${STATUS_CONFIG[col.key].dot}` }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    ...
                    {colReels.map(reel => (
                      <div key={reel.id} style={{ background: 'var(--card)', border: '1px solid var(--card-border)', borderRadius: '10px', padding: '1rem' }}>
                        <p style={{ fontFamily: "'Instrument Serif', serif", fontSize: '0.95rem', fontWeight: 400, lineHeight: 1.3, marginBottom: '0.5rem' }}>{reel.title}</p>
                        <p style={{ fontSize: '0.58rem', color: 'var(--muted)', lineHeight: 1.6, marginBottom: '0.75rem' }}>{reel.topic}</p>
                        {reel.notes && <p style={{ fontSize: '0.55rem', color: 'var(--faint)', fontStyle: 'italic', marginBottom: '0.75rem' }}>{reel.notes}</p>}
                        ...
                        {reel.stats && (
                          <div style={{ marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px solid var(--border)', display: 'flex', gap: '0.75rem' }}>
                            <div><div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '0.9rem' }}>{reel.stats.views.toLocaleString()}</div><div style={{ fontSize: '0.45rem', color: 'var(--faint)', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>views</div></div>
                            <div><div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '0.9rem' }}>{reel.stats.shares.toLocaleString()}</div><div style={{ fontSize: '0.45rem', color: 'var(--faint)', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>shares</div></div>
                            <div><div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '0.9rem' }}>{reel.stats.saves.toLocaleString()}</div><div style={{ fontSize: '0.45rem', color: 'var(--faint)', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>saves</div></div>
                          </div>
                        )}
                        {(reel.videoLink || reel.brollLinks?.length > 0) && (
              ...
              return (
                <div key={reel.id} style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: '2rem', padding: '2.5rem 0', borderBottom: '1px solid var(--border)' }}>
                  <div style={{ paddingTop: '0.2rem' }}>
                    <span style={{ fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase' as const, color: 'var(--faint)' }}>#{String(i + 1).padStart(2, '0')}</span>
                  </div>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' as const, marginBottom: '0.75rem' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: 'var(--muted)' }}>
                    ...
                    <h2 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: '1.6rem', letterSpacing: '-0.01em', lineHeight: 1.1, marginBottom: '0.5rem' }}>{reel.title}</h2>
                    ...
                    {reel.stats && (
                      <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '1.5rem', marginTop: '1.25rem', padding: '1rem', background: 'var(--card)', border: '1px solid var(--card-border)', borderRadius: '8px' }}>
                        {[
                          ...
                          <div key={s.label}>
                            ...
                            <div style={{ fontSize: '0.5rem', color: 'var(--faint)', letterSpacing: '0.1em', textTransform: 'uppercase' as const }}>{s.label}</div>
                          </div>
                        ))}
                      </div>
                    )}
                    <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '0.75rem', marginTop: '1rem' }}>
                      {reel.videoLink && <a href={reel.videoLink} target="_blank" rel="noopener noreferrer" style={linkStyle()}>video ↗</a>}
                      {reel.audioLink && <a href={reel.audioLink} target="_blank" rel="noopener noreferrer" style={linkStyle()}>audio ↗</a>}
                      {reel.brollLinks?.map((l, j) => <a key={j} href={l} target="_blank" rel="noopener noreferrer" style={linkStyle(true)}>b-roll {j+1} ↗</a>)}
                    </div>
                  </div>
                </div>
              )
      ...
      <section style={{ maxWidth: '1200px', width: '100%', margin: '0 auto', padding: '0 2.5rem 4rem', borderTop: '1px solid var(--border)' }}>
        <div style={{ paddingTop: '3rem' }}>
          <button onClick={() => setRulesOpen(!rulesOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: 0, width: '100%', textAlign: 'left' as const, color: 'var(--fg)' }}>
            <span style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: '1.4rem' }}>Ray&apos;s {rulesData.length} rules for growth</span>
          ...
          <p style={{ fontSize: '0.65rem', color: 'var(--muted)', marginTop: '0.5rem' }}>learned from your data. updated every 10 reels.</p>
          {rulesOpen && (
            <div style={{ marginTop: '2rem', display: 'flex', flexDirection: 'column' }}>
              {rulesData.map((rule, i) => (
                <div key={rule.id} style={{ display: 'grid', gridTemplateColumns: '48px 1fr', gap: '1.5rem', padding: '1.75rem 0', borderTop: '1px solid var(--border)' }}>
                  <span style={{ fontSize: '0.6rem', letterSpacing: '0.12em', color: 'var(--faint)', paddingTop: '0.15rem' }}>{String(i + 1).padStart(2, '0')}</span>
                  <div>
